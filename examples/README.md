...
- Configura:
  ...
  - SLACK_BOT_TOKEN si usarás notificaciones.
- Sustituye los nodos "stub" por OCR real (AWS Textract o Google Document AI) si lo necesitas.

Flujo:
Webhook → Normalize → (Download stub) → (OCR stub) → OpenAI → Parse → Guardrails → If Review → [Wait/Webhook] → Merge → Postgres → Slack → Respond
